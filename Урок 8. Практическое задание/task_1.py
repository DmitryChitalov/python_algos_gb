...
class Branches(namedtuple("Branches", ["left", "right"])):
    def walk(self, code, acc):
        self.left.walk(code, acc +
                       '0')  # пойти в левую ветвь, добавив к префиксу "0"
        self.right.walk(code, acc +
                        '1')  # пойти в правую ветвь, добавив к префиксу "1"
...
class Leaves(namedtuple("Leaves", ["char"])):
    def walk(self, code, acc):
        code[
            self.
            char] = acc or "0"  # если строка длиной 1 то acc = "", для этого случая установим значение acc = "0"
...
def haffman_encode(s):
    ...
    while len(my_lst) > 1:
        ...
        heapq.heappush(my_lst,
                       (weight_1 + weight_2, count, Branches(left, right)))
        count += 1
    """
