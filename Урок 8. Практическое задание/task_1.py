...
"""
Кучи - это двоичные деревья, для которых каждый родительский узел имеет значение, меньшее или равное любому из его 
дочерних элементов. В этой реализации используются массивы, для которых heap[k] <= heap[2*k+1] и heap[k] <= heap[2*k+2] 
для всех k, считая элементы с нуля. Для сравнения, несуществующие элементы считаются бесконечными. Интересным свойством 
кучи является то, что ее наименьшим элементом всегда является корень heap[0].
"""
...
"""
Решение не моё личное, нашёл, порывшись в интернете, самостоятельно не получалось правильно выстроить бинарное дерево
благодаря данному примеру разобрался и решил оставить его, немного изменив под текущую версию питона. С кучами так же
разобрался по модулю heapq, считаю, что отлично подходят для решения.
"""