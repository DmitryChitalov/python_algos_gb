...
def haffmans_tree(s):
    ...
    if len(sorted_elements) != 1:
        ...
        while len(sorted_elements) > 1:
            ...
            '''
            combined = {0: sorted_elements.popleft()[0],
            ...
            for i, _count in enumerate(sorted_elements):
                if weight > _count[1]:
                    ...
                    # deque([({0: 'r', 1: '!'}, 2), ('p', 2), (' ', 2), ('o', 2), ('b', 3), ('e', 4)])
                    sorted_elements.insert(i, (combined, weight))
                    break
            else:
                # Добавляем объединенный корневой элемент после
                # завершения работы цикла

                sorted_elements.append((combined, weight))
            '''
            deque([({0: 'r', 1: '!'}, 2), ('p', 2), (' ', 2), ('o', 2), ('b', 3), ('e', 4)])
        ...
        combined = {0: sorted_elements.popleft()[0], 1: None}
        sorted_elements.append((combined, weight))
...
def haffmans_code(tree, path=''):
    ...
    # по первому и второму значению (левая и правая ветви).
        ...
        haffmans_code(tree[0], path=f'{path}0')
        haffmans_code(tree[1], path=f'{path}1')
...
print('Скорость кодирования строки по Хаффману',
      timeit.timeit(
          "haffmans_code(haffmans_tree(s))",
          globals=globals(),
          number=1000))
