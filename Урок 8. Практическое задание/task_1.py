...
def grow_trees(seed):
    ...
    while len(rates.keys()) > 1:
        ...
        if not isinstance(right[0], type('str')):
            right[0].code = 1
        new_node = BinNodes(cur_value, '', BinNodes(left[0], 0) if isinstance(left[0], type('str')) else left[0],
                            BinNodes(right[0], 1) if isinstance(right[0], type('str')) else right[0])
...
my_st = 'python is a best language like a JavaScript'
my_tree = grow_trees(my_st)
