...
from collections import namedtuple  # реализация на именованном кортеже


def calc():
    my_var = "Company"  # название компании
    n = int(input("Введите количество предприятий: "))
    # создаем коллекцию именованный кортеж:
    companies = namedtuple(my_var, " name period_1 period_2 period_3 period_4")
    profit_aver = {}  # задаем пустое множество для
    # передаем данные объекту созданной коллекции:
    for i in range(n):
        company = companies(name=input("Введите название предприятия: "),
                            ...
                            period_4=int(input(
                                "Введите прибыль за четвертый квартал: ")))
        # достаем из объекта внесенные значения и считаем средн. прибыль за год
        profit_aver[company.name] = (company.period_1 + company.period_2 +
    ...
    # сравниваем со средней прибылью и выводим результат
