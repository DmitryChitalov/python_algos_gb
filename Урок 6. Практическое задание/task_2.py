...
"""
Исользование модуля struct дает выигрыш в памяти при хранении данных (например экземпляров класса). В примере ниже
в class StructPolygon мы сохраняем все данные экземпляра класса в одном объекте ('_data',) (используя __slots__), а 
...
копирования. Из профилирования видно, что при использовании Memoryview не расходуется доп. память, плюс срезы 
...
def usual_slice():
    for n in (100000, 200000, 300000, 400000):
        ...
        while b:
            b = b[1:]
        print(f'{n} байт - время выполнения {time.time() - start} сек.')
