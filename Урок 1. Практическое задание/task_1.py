...
def check_1(lst_obj):
    ...
    Сложность: O(N).
    Так как операции по добавлению или удалению или поиску значения в множестве имеют сложность
    O(1) то общая сложность будет зависеть от количества элементов в списке
    """
...
def check_3(lst_obj):
    ...
    Сложность: O(N Log N) самая затратная операция соответсвенно остальные отбрасываем
    """
