...
def check_1(lst_obj):
    ...
    Сложность: O(len(lst_obj)+1) или O(n).
    """
...
def check_2(lst_obj):
    ...
    Сложность: O(len(lst_obj) * len(lst_obj)/2*2 + 1) или O(n^2).
    """
    for j in range(len(lst_obj)):          # O(len(lst_obj))
        if lst_obj[j] in lst_obj[j+1:]:    # O(len(lst_obj)/2) на срез и столько же на поиск вхождения(in) + O(1) за lst_obj[j]
            return False                   # O(1) - выполнится один раз это
    return True                            # O(1) или это выражение
...
def check_3(lst_obj):
    ...
    Сложность: O(n) + O(n*log(n)) + O(n) + 1 или O(n*log(n))
    """
    lst_copy = list(lst_obj)                 # O(n), где  n = len(lst_obj)
    lst_copy.sort()                          # O(n*log(n))
    for i in range(len(lst_obj) - 1):        # O(n-1) (можно принять за O(n))
