...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(n)
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) - работа на извлечение среза из массива
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    for i in range(len(lst_obj) - 1):        # O(n)
        if lst_copy[i] == lst_copy[i+1]:     # O(1) -чтение из массива конкретного элемента и сравнение
            return False                     # O(1)
    return True                              # O(1)
