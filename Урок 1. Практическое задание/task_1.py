...
def check_1(lst_obj):
    ...
    Сложность: O(n).
    """
    lst_to_set = set(lst_obj)  # O(n)  Итерируемся по списку, создавая множество
    return lst_to_set
...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(n)
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) + O(n) = O(n)  (in + slice)
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    lst_copy.sort()                          # O(n*log n) (using quicksort)
