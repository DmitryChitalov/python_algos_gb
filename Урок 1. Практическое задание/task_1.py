...
def check_2(lst_obj):
    ...
    Сложность: T(n) = 1n^2 + 1 Квадратичная (по первому элементу)
    """
...
def check_3(lst_obj):
    ...
    Сложность: T(n) = n + n*log(n) + 2*n + 1 Линейно-логарифмическая (по второму элементу)
    """
    lst_copy = list(lst_obj)                 # Линейная сложность O(n)
    lst_copy.sort()                          # Линейно-логарифмическая O(n*log n)
    for i in range(len(lst_obj) - 1):        # Линейная сложность O(n)
        if lst_copy[i] == lst_copy[i+1]:     # Константная сложность O(1)
            return False                     # Константная сложность O(1)
    return True                              # Константная сложность O(1)
