...
def check_2(lst_obj):
    ...
    Сложность: O(N**2).
    """
    for j in range(len(lst_obj)):  # O(N + 1) = O(N)
        if lst_obj[j] in lst_obj[j + 1:]:  # O(max(N,N,1,1)) = O(N)
            return False  # O(1)
    return True  # O(1)
...
for rand in (50, 500, 1000, 5000, 1000):
    # Из 100000 чисел возьмем 'j' случайно выбранных
    # Всего 10 тыс. чисел
    lst = random.sample(range(-100000, 100000), rand)
