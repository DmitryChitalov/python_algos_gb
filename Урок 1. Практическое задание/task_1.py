...
def check_1(lst_obj):
    ...
    Сложность: O(n).
    """
    lst_to_set = set(lst_obj)  # Скорей всего O(n) так-как множестово сортирует все повторящиися элементы
    return lst_to_set
...
def check_3(lst_obj):
    ...
    lst_copy.sort()                          # O(n log n). Изначально думал что сложность просто O(log n)
