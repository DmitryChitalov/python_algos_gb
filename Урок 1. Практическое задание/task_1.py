...
def check_1(lst_obj):
    ...
    Сложность: сложность О(len()).
    """
    lst_to_set = set(lst_obj)  # сложность О(len()) зависит от длины аргумента (табл 6.1.2)
    return lst_to_set    # сложность O(1) константная
...
def check_2(lst_obj):
    ...
    """
    for j in range(len(lst_obj)):          # O(n)  - перебор индексов списка
...
def check_3(lst_obj):
    ...
    Сложность: O(nlog n) линейно-логарифмическая
    """
