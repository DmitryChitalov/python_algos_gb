...
def check_2(lst_obj):
    ...
    Сложность: O(n^2)
    """
    for j in range(len(lst_obj)):          # O(n) -> линейный цикл
        if lst_obj[j] in lst_obj[j+1:]:    # O(n) ->поиск в списке + O(n) -> срез
            return False                   # O(1) -> по идее здесь O(n), т.к. он возвращает за каждый шаг
    return True                            # O(1) -> один возврат
...
def check_3(lst_obj):
    ...
    Сложность: O(nLog n)
    """
    lst_copy = list(lst_obj)                 # O(n)-> присваивание списка + O(len(lst_obj))
...
for j in (50, 500, 1000, 5000, 1000):
    # Из 100 000 чисел возьмем 'j' случайно выбранных
    # Всего 10 тыс. чисел
