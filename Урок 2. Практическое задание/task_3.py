...
def reverse_number_recursive(current_number: int):
    if current_number < 0:
        raise Exception('Число должно быть неотрицательным')
    current_digit = current_number % 10
    number_without_current_digit = current_number // 10
    if number_without_current_digit == 0:
        return str(current_digit)
    else:
        return f"{current_digit}{reverse_number_recursive(number_without_current_digit)}"


number = int(input("Введите число, которое требуется перевернуть > "))
print(f"Введённое число: {number}")
print(f'Перевёрнутое число: {reverse_number_recursive(number)}')
